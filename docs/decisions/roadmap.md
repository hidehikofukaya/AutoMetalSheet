...
### 6.4.3 候補集合の被覆(2026-09-06)と 7 族への拡張

- **候補の多様性は潰れていない、むしろ増えた**: occt13 で 9 本の候補が「教師と変種の両方」を 3mm 以内で被覆する部品は、
  基準 30% → **集合値 90%**。「変種だけ」に潰れた部品は 33% → **0%**。集合値の教師は片方に寄るのではなく**両方を出せる**ようになる
- 新 3 族を取り込み(8207 部品 = CATIA 2807 + OCCT 5400、往復 1600/1600、忠実度 0.01mm、締結点 3〜8)。教師床: 座面 100%、
  自己整合 0%(occt17 平板は面 1・辺 8、occt18/19 は辺 24・余剰 680〜700°)
- 次の学習: 7 族 5160 部品、`--set-target --ema`、1800 ep × 2 シード(`sweep_frame7.json`)。バリアントは 5 族分(2400)。
...
- occt20(tab_bracket、締結点 4〜5、600 部品、変種 2,439)を取り込み → 9 族 6,000 部品、変種 33,533。取り込みスクリプトの
  ワイヤーフレーム複写が `o1*` 固定で occt20 を落としていた(面教師が空、スペック欠落)→ `o*` に修正して再生成。
  **教訓**: 取り込み後は「タグ別の件数」だけでなく面教師の中身(x2a/x2b の形)とスペック件数を確認する
