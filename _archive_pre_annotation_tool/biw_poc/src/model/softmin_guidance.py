...
class SoftminGuidanceModel(nn.Module):
    ...
    def __init__(
        ...
        use_thickness_in_encoder: bool = False,
        dropout: float = 0.0,
        n_freqs: int = 8,
    ):
        super().__init__()
        ...
        # Defaults to False so loading a checkpoint saved before this
        # conditioning existed (whose architecture metadata omits the key)
        # reconstructs a model with no thickness_enc submodule, matching its
        # state_dict exactly. New training runs opt in via TrainConfig.
        self.use_thickness = use_thickness_in_encoder
        self._architecture = {
            "token_dim": token_dim,
            "n_tokens": n_tokens,
            ...
            "use_thickness_in_encoder": use_thickness_in_encoder,
            "dropout": dropout,
            "n_freqs": n_freqs,
        }
        ...
        self.thickness_enc: nn.Sequential | None = None
        if use_thickness_in_encoder:
            hidden = max(token_dim // 4, 1)
            self.thickness_enc = nn.Sequential(
                nn.Linear(1, hidden),
                nn.GELU(),
                nn.Linear(hidden, token_dim),
            )
            # Zero-init the last layer so thickness conditioning starts as an
            # exact no-op (mirrors the decoder head[-1] zero-init above) and
            # only diverges from that as training demands.
            with torch.no_grad():
                self.thickness_enc[-1].weight.zero_()
                self.thickness_enc[-1].bias.zero_()
    ...
    def _validate_encoder_inputs(
        ...
        thickness_norm: torch.Tensor | None = None,
    ) -> None:
        ...
        if self.use_thickness:
            if thickness_norm is None:
                raise ValueError(
                    "thickness_norm is required when use_thickness_in_encoder=True"
                )
            if thickness_norm.ndim != 1 or thickness_norm.shape[0] != points.shape[0]:
                raise ValueError(
                    "thickness_norm must have shape [B] matching points' batch size, "
                    f"got {tuple(thickness_norm.shape)}"
                )
    ...
    def encode(
        ...
        self._validate_encoder_inputs(points, normals, thickness_norm)
        ...
        if self.use_thickness:
            thickness_embed = self.thickness_enc(thickness_norm.view(-1, 1, 1))
            tokens = tokens + thickness_embed
    ...
    def forward(
        ...
        latent = self.encode(points, normals, thickness_norm)
    ...
    def checkpoint_metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "input_contract": {
                ...
                "thickness_norm": (
                    "thickness_mm / scale_mm" if self.use_thickness else "unused"
                ),
