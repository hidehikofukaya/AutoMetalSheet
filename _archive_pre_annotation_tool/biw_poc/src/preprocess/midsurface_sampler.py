...
--------------------
  body###_filled.stp   (hole-filled solid; run hole_filler.py first -- R6-03)
    ...
    -> [A-4] noisy training input point cloud
             (area-weighted resample to N pts + Gaussian noise; an optional
             random spherical occlusion mask is available via
             --occlusion-prob but is OFF by default -- R-coverage-01: the
             default path guarantees whole-part GT coverage with no
             permanent gaps. See the occlusion_prob docstring on
             sample_training_cloud() for why it must stay opt-in.)
...
def sample_training_cloud(tri_mesh: trimesh.Trimesh,
    ...
    """Area-weighted surface sample + Gaussian noise.

    ``occlusion_prob`` is OFF by default (R-coverage-01). It is kept as an
    opt-in knob for future partial-scan-robustness experiments, but it must
    not be enabled casually: with the current ``oversample = 1.5 * n_points``
    and ``mask_ratio_range`` up to 0.3, a triggered occlusion always leaves
    ``>= 0.7 * oversample > n_points`` points remaining, so execution always
    takes the downsample branch below and NEVER the zero-pad branch. The
    masked region is therefore a permanent, unfilled gap in the returned
    cloud -- confirmed on body002 to leave ~27% of the GT surface (one
    contiguous blob) more than 5mm from any sampled point. Re-enabling this
    knob requires either fixing the pad branch to resample fresh points
    (rather than duplicate survivors) or accepting that whole-part coverage
    is intentionally sacrificed for that run.
    """
    ...
    if occlusion_prob > 0 and rng.random() < occlusion_prob:
...
def main():
    ...
    ap.add_argument("--occlusion-prob", type=float, default=0.0,
                    help="R-coverage-01 (opt-in, default disabled): probability of "
                         "applying a synthetic spherical occlusion mask to the input "
                         "point cloud in sample_training_cloud(). Off by default because "
                         "the post-occlusion re-pad path is unreachable given the current "
                         "oversample/mask_ratio settings, so masked regions are never "
                         "backfilled and become permanent GT coverage gaps (confirmed on "
                         "body002: roughly a quarter of the surface left uncovered as one "
                         "contiguous blob). Re-enabling this requires first fixing that pad-branch "
                         "bug. See CLAUDE.md section 19 / sample_training_cloud() docstring.")
